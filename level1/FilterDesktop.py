...
class FilterDesktop(unittest.TestCase):
    ...
    def test(self):
        ...
        if data['price_min']:
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-0']/div/div[2]/input").clear()
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-0']/div/div[2]/input").send_keys(str(data['price_min']))
        if data['price_max']:
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-0']/div/div[2]/input[2]").clear()
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-0']/div/div[2]/input[2]").send_keys(str(data['price_max']))
        ...
        if data['search']:
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-3']/div/input").clear()
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-3']/div/input").send_keys(str(data['search']))
            driver.find_element_by_xpath("//div[@id='mz-filter-panel-0-3']/div/input").send_keys(Keys.ENTER)
        time.sleep(4)
        if data['is_empty']:
